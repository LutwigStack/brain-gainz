...
export const resolveRuntimeProfile = ({
  ...
  if (capacitorNative) {
    return createRuntimeProfile({
      ...
      dataBoundaryLabel: 'Отдельное хранилище этой оболочки. Данные не синхронизируются с браузером или десктопом.',
      isLocalFirst: false,
      usesNativeSql: false,
  ...
  if (tauriInvoke) {
    return createRuntimeProfile({
      ...
      dataBoundaryLabel: 'Отдельная десктопная база. Данные из браузерного режима сюда не синхронизируются.',
      isLocalFirst: true,
      usesNativeSql: true,
  ...
  return createRuntimeProfile({
    ...
    dataBoundaryLabel: 'Отдельное браузерное хранилище. Данные из десктопного режима сюда не синхронизируются.',
    isLocalFirst: false,
    usesNativeSql: false,
    usesSafeAreaInsets: false,
    desktopOnlyCapabilities: [],
